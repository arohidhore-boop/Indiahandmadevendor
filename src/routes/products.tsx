...
import { AppShell, StatusPill } from "@/components/ih/AppShell";
import { useIH } from "@/lib/ih-store";

export const Route = createFileRoute("/products")({
  head: () => ({ meta: [{ title: "Products — India Handmade" }] }),
  component: Products,
});
            ...
            <li key={p.id} className="grid grid-cols-[1.7fr_120px_90px_140px_80px] items-center px-6 py-4 text-sm">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-lg bg-[var(--cream)] grid place-items-center text-xl shrink-0 overflow-hidden">
                  {p.imageUrl ? <img src={p.imageUrl} alt="" className="h-full w-full object-cover" /> : (p.emoji ?? "📦")}
                </div>
                <div>
