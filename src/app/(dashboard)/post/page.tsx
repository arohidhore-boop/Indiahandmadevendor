...
import { SellerDashboard } from "@/components/ih/seller-dashboard";
import { useStore, type OnboardingStatus, type BankStatus } from "@/lib/store";
...
function QuickAction({ icon: Icon, label, href }: { icon: typeof Check; label: string; href: string }) {
  return (
    <Link href={href} className="surface-card p-5 flex items-center gap-3 hover-lift group">
...
export default function Home() {
  ...
  const { seller, onboarding } = useStore();
  ...
  const dismissOverlay = () => {
    if (typeof window !== "undefined") sessionStorage.removeItem("justRegistered");
    setJustRegistered(false);
  };

  const statuses = onboarding.statuses;
  const profileCompleted = !justRegistered && PROFILE_STEPS.every((s) => statuses[s.n] === "completed");
  const completedSteps = justRegistered
    ? 0
    : PROFILE_STEPS.filter((s) => statuses[s.n] === "completed").length;
  ...
  const displayStatuses = (justRegistered
    ...
    : statuses) as Record<number, OnboardingStatus>;
  ...
  if (showSellerDashboard && !justRegistered) {
  ...
  return (
    <>
      ...
      <div>
        <h1 className="font-serif text-[28px] leading-[32px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 -mt-3">
        <div className="lg:col-span-2 flex flex-col gap-5">
          <div className="surface-card p-6 sm:p-7">
            ...
            <ul className="mt-6 divide-y divide-[var(--border)] border border-[var(--border)] rounded-2xl overflow-hidden">
              {PROFILE_STEPS.map((s) => {
                const status = displayStatuses[s.n] ?? "not_started";
                const Icon = s.icon;
                return (
                  <li key={s.n}>
                    <Link
                    ...
                    >
                      ...
                      <StatusBadge status={status} bankStatus={s.n === 4 ? onboarding.data.bankStatus : undefined} />
              ...
              <li>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[var(--muted-foreground)] mt-0.5 truncate">Available after profile completion</div>
                  </div>
                  {firstProductDone ? (
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              {!profileCompleted ? (
                <Link href={`/onboarding?step=${nextStep?.n ?? 1}`} className="ih-btn ih-btn-primary">
        ...
        <div className="flex flex-col gap-4">
          <QuickAction icon={BookOpen} label="View seller guide" href="/help" />
        </div>
      </div>
    </>
  );
