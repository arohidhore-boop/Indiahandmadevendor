  ...
  return (
    <AuthShell>
      <div className="surface-card p-8">
        ...
        <p className="text-sm text-[var(--muted-foreground)] mt-1">Sign in to continue selling on India Handmade.</p>
        <form onSubmit={(e) => { e.preventDefault(); router.push("/post"); }} className="mt-6 space-y-4">
          <Field label="Email">
          ...
          <Field label="Password">
            <div className="relative">
              ...
              <button type="button" onClick={() => setShow((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)]" aria-label={show ? "Hide password" : "Show password"}>
          ...
          <button type="submit" className="ih-btn ih-btn-primary ih-btn-full">Sign In</button>
        </form>
        <p className="mt-6 text-sm text-center text-[var(--muted-foreground)]">
          New here?{" "}
          <Link href="/signup" className="text-[var(--primary)] font-medium hover:underline">Create an account</Link>
        </p>
      </div>
    </AuthShell>
  );
